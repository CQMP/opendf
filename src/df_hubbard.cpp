...
typename df_hubbard<LatticeT>::gw_type df_hubbard<LatticeT>::operator()(alps::params p)
{
    ...
    std::vector<std::complex<double>> reduced_sigma_d(sigma_d_.size());
    std::vector<std::complex<double>> vector_sigma_d(sigma_d_.size());
    ...
    for (size_t nd_iter=0; nd_iter<df_sc_iter && diff_gd > df_sc_cutoff; ++nd_iter) { 
        ...
        for (int Windex = start_Windex; Windex <= end_Windex; Windex++) { 
            ...
            MPI_Allreduce(vector_sigma_d.data(),reduced_sigma_d.data(),size_sigma,MPI_DOUBLE_COMPLEX,MPI_SUM,MPI_COMM_WORLD); 
    ...
    if (store_full_diag_vertex) { 
       ...
       std::vector<std::complex<double>> vector_full_vertex(sigma_d_.size());
       std::vector<std::complex<double>> reduced_full_vertex(sigma_d_.size());
       ...
       std::fill(reduced_full_vertex.data(),&reduced_full_vertex.back(),czero);
       int jj=0;
       ...
       MPI_Allreduce(vector_full_vertex.data(),reduced_full_vertex.data(),size_full_vertex,MPI_DOUBLE_COMPLEX,MPI_SUM,MPI_COMM_WORLD);
